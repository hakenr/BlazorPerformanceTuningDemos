...
namespace BlazorPerformanceTuningDemos.Client.DemoData;
...
public class DemoDataService : IDemoDataService
{
	...
	public async Task<IEnumerable<EmployeeDto>> GetEmployeesDataFragmentAsync(int startIndex, int? count, CancellationToken cancellationToken = default)
	{
		...
		return _employees.Skip(startIndex).Take(count ?? int.MaxValue).ToList();
	}
	...
	public async Task<IEnumerable<EmployeeDto>> GetEmployeesDataFragmentAsync(EmployeesFilterDto filter, int startIndex, int? count, CancellationToken cancellationToken = default)
	{
		...
		return GetFilteredEmployees(filter).Skip(startIndex).Take(count ?? int.MaxValue).ToList();
	}
	...
	public async Task<EmployeeDto> GetEmployeeDetailAsync(int employeeId, CancellationToken cancellationToken = default)
	{
		_logger.LogInformation($"DemoDataService.GetEmployeeDetailAsync(\"{employeeId}\") called.");

		await Task.Delay(1000, cancellationToken); // simulate server call
